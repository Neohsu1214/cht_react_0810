...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    default:
      // 這個 default 很重要！！不然會狂報錯！！當以上皆非時，就直接回傳state
      return state;
  }
};
...
unsubscribe1 = store.subscribe(()=>{ console.log('monitor change, state=', store.getState())})

// perform something in store
console.log('David buy a coke')
store.dispatch(buyCoke())
console.log('John buy two cokes')
store.dispatch(buyCoke())
store.dispatch(buyCoke())
unsubscribe1() // 取消監聽1
unsubscribe2 = store.subscribe(()=>{console.log('** change, state=', store.getState())}) // 改用新的監聽
console.log("Mary buy 2 cokes:")
store.dispatch(buyCoke())
store.dispatch(buyCoke())
store.dispatch(buyCoke())
unsubscribe2() // 取消監聽2
